...
extern float       powf_special(float x, float y);
#pragma CODE_SECTION(DCL_runNLPID_C2, "ramfuncs");
#pragma CODE_SECTION(DCL_runNLPID_C1, "ramfuncs");
...
//!
typedef volatile struct _nlpid_sps {
...
//!
typedef volatile struct _nlpid {
    float32_t Kp;       //!< Linear proportional gain
    float32_t Ki;       //!< Linear integral gain
    float32_t Kd;       //!< Linear derivative gain
    float32_t alpha_p;  //!< P path non-linear exponent
...
{
    return((float32_t) powf_special(delta, (alpha - 1.0f)));
}
...
{
    return((float32_t) powf_special(gamma, (1.0f / (alpha - 1.0f))));
}
...
{
    float32_t xP = (float32_t) powf_special(p->sps->delta_p, (p->sps->alpha_p - 1.0f));
    float32_t xI = (float32_t) powf_special(p->sps->delta_i, (p->sps->alpha_i - 1.0f));
    float32_t xD = (float32_t) powf_special(p->sps->delta_d, (p->sps->alpha_d - 1.0f));
...
static inline float32_t DCL_runNLPID_C1(DCL_NLPID *p, float32_t rk, float32_t yk, float32_t lk)
{
    ...
    // non-linear modules
    v4 = ((v3 > p->delta_p) ? (v2 * (float32_t) powf_special(v3, p->alpha_p)) : (v1 * p->gamma_p));
    v5 = ((v3 > p->delta_i) ? (v2 * (float32_t) powf_special(v3, p->alpha_i)) : (v1 * p->gamma_i));
    v9 = ((v3 > p->delta_d) ? (v2 * (float32_t) powf_special(v3, p->alpha_d)) : (v1 * p->gamma_d));
...
static inline float32_t DCL_runNLPID_C2(DCL_NLPID *p, float32_t rk, float32_t yk, float32_t lk)
{
    ...
    v6 = ((vd3 > p->delta_d) ? (vd2 * (float32_t) powf_special(vd3, p->alpha_d)) : (yk * p->gamma_d));
...
//--- Non-linear PI controllers -----------------------------------------------

//! \brief          Defines the shadow DCL_NLPI controller structure
//!
...
//! \brief          Defines default values to initialize the DCL_NLPI structure
//!
#define NLPI_SPS_DEFAULTS { 1.0f, 0.0f, \
                            1.0f, 1.0f, \
                            0.1f, 0.1f, \
                            1.0f, 1.0f, \
                            1.0f, -1.0f }

//! \brief          Defines the active DCL_NLPI controller structure
//!
typedef volatile struct {
    float32_t Kp;       //!< Linear proportional gain
    float32_t Ki;       //!< Linear integral gain
    ...
    float32_t gamma_i;  //!< I path gain limit
    float32_t i7;       //!< I path intermediate storage
    float32_t i11;      //!< Intermediate saturation storage
    float32_t i12;      //!< Spare
    float32_t Umax;     //!< Upper saturation limit
    float32_t Umin;     //!< Lower saturation limit
    DCL_NLPI_SPS *sps;  //!< Pointer to shadow parameter structure
    DCL_CSS *css;       //!< Pointer to controller support structure
} DCL_NLPI;

//! \brief          Defines default values to initialize the DCL_NLPI structure
//!
#define NLPI_DEFAULTS {     1.0f, 0.0f, \
                            1.0f, 1.0f, \
                            0.1f, 0.1f, \
                            1.0f, 1.0f, \
                            0.0f, 1.0f, 0.0f, \
...
//! \brief          Resets NLPI internal storage data
//! \param[in] p    Pointer to the DCL_NLPI structure
//! \return         None
//!
static inline void DCL_resetNLPI(DCL_NLPI *p)
{
    uint16_t v;
...
//! \brief          Loads active NLPI controller parameters
//! \param[in] p    Pointer to the active DCL_NLPI controller structure
//! \return         None
//!
static inline void DCL_updateNLPI(DCL_NLPI *p)
{
    uint16_t v;
    ...
    p->css->err |= (p->sps->delta_i < DCL_DELTA_MIN) ? ERR_PARAM_RANGE : ERR_NONE;
    p->css->err |= (p->sps->Umax <= p->sps->Umin) ? ERR_PARAM_INVALID : ERR_NONE;
    p->css->err |= ((p->sps->Kp < 0.0f) || (p->sps->Ki < 0.0f)) ? ERR_PARAM_RANGE : ERR_NONE;
    if (p->css->err)
    {
        DCL_GET_ERROR_LOC(p->css);
        DCL_RUN_ERROR_HANDLER(p->css);
...
//! \brief          Executes an inline series form NLPI controller on the FPU32
//! \param[in] p    Pointer to the DCL_NLPI structure
//! \param[in] rk   The controller set-point reference
//! \param[in] yk   The measured feedback value
//! \return         The control effort
//!
static inline float32_t DCL_runNLPI_C1(DCL_NLPI *p, float32_t rk, float32_t yk)
{
    float32_t v1, v2, v3, v4, v5, v8, v9, v10;
    ...
    // integral path
    v8 = (v5 * p->Kp * p->Ki * p->i11) + p->i7;
    p->i7 = v8;

    // output sum & clamp
    v9 = (p->Kp * v4) + v8;
#ifndef __TMS320C28XX_CLA__
    v10 = __fsat(v9, p->Umax, p->Umin);
#else
    v10 = (v9 > p->Umax) ? p->Umax : v9;
    v10 = (v10 < p->Umin) ? p->Umin : v10;
#endif
    p->i11 = (v10 == v9) ? 1.0f : 0.0f;
...
static inline float32_t DCL_runNLF_C1(float32_t x, float32_t alpha, float32_t delta)
{
    ...
    return((v3 > delta) ? (v2 * (float32_t) powf_special(v3, alpha)) : (x * powf_special(delta, (alpha - 1.0f))));
}
