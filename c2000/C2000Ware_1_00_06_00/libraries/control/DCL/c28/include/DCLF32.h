...
#pragma CODE_SECTION(DCL_runClamp_C2, "ramfuncs");
...
//!
typedef volatile struct _lpid_sps {
...
//!                 Only z1, z2 & p2 considered.  p1 = 1 assumed.
//! \param[in] p    Pointer to the active DCL_PID controller structure
...
{
    float32_t   tau, b2, b1, b0, detA, Vi, Vd;
    ...
    p->sps->c2 = -q->p2;
    tau = -(p->css->T / 2.0f) * (p->sps->c2 - 1) / (p->sps->c2 + 1);
    p->sps->c1 = 2.0f / (p->css->T + 2.0f * tau);
    b2 = q->K;
    b1 = -q->K * (float32_t) crealf(q->z1 + q->z2);
    b0 = q->K * (float32_t) crealf(q->z1 * q->z2);
    detA = 1.0f + (2.0f * p->sps->c2) + (p->sps->c2 * p->sps->c2);
    p->sps->Kp = ((p->sps->c2 * b2) - b1 - ((p->sps->c2 + 2.0f) * b0)) / detA;
    Vi = (p->sps->c2 + 1.0f) * (b2 + b1 + b0) / detA;
    Vd = ((p->sps->c2 * p->sps->c2 * b2) - (p->sps->c2 * b1) + b0) / detA;
    p->sps->Ki = Vi / p->sps->Kp;
    p->sps->Kd = Vd / p->sps->Kp;
}
...
static inline void DCL_loadParallelPIDasZPK(DCL_PID *p, DCL_ZPK3 *q)
{
    ...
    detA = 1.0f + (2.0f * p->sps->c2) + (p->sps->c2 * p->sps->c2);
    p->sps->Kp = ((p->sps->c2 * b2) - b1 - ((p->sps->c2 + 2.0f) * b0)) / detA;
    p->sps->Ki = (p->sps->c2 + 1.0f) * (b2 + b1 + b0) / detA;
    p->sps->Kd = ((p->sps->c2 * p->sps->c2 * b2) - (p->sps->c2 * b1) + b0) / detA;
}
...
//!                 Zero frequency assumed to be in Hz
...
static inline void DCL_loadSeriesPIasZPK(DCL_PI *p, DCL_ZPK3 *q)
{
#ifdef DCL_ERROR_HANDLING_ENABLED
    p->css->err |= (q->K < 0.0f) ? ERR_PARAM_RANGE : ERR_NONE;
    p->css->err |= (crealf(q->z1) < 0.0f) ? ERR_PARAM_RANGE : ERR_NONE;
    ...
    p->sps->Kp = q->K * (float32_t) crealf(q->z1);
    p->sps->Ki = (q->K / p->sps->Kp) - 1.0f;
}
...
static inline void DCL_loadDF11asZPK(DCL_DF11 *p, DCL_ZPK3 *q)
{
    ...
    p->sps->b0 = q->K;
    p->sps->b1 = -q->K * (float32_t) crealf(q->z1);
    p->sps->a1 = (float32_t) -crealf(q->p1);
}
...
static inline void DCL_loadDF13asZPK(DCL_DF13 *p, DCL_ZPK3 *q)
{
    ...
    p->sps->b0 = q->K;
    p->sps->b1 = -q->K * (float32_t) crealf(q->z1 + q->z2 + q->z3);
    p->sps->b2 = q->K * (float32_t) crealf((q->z1 * q->z2) + (q->z1 * q->z3) + (q->z2 * q->z3));
    p->sps->b3 = -q->K * (float32_t) crealf(q->z1 * q->z2 * q->z3);
    p->sps->a0 = 1.0f;
    p->sps->a1 = (float32_t) -crealf(q->p1 + q->p2 + q->p3);
    p->sps->a2 = (float32_t) crealf((q->p1 * q->p2) + (q->p1 * q->p3) + (q->p2 * q->p3));
    p->sps->a3 = (float32_t) -crealf(q->p1 * q->p2 * q->p3);
}
...
static inline void DCL_loadDF22asZPK(DCL_DF22 *p, DCL_ZPK3 *q)
{
    ...
    p->sps->b0 = q->K;
    p->sps->b1 = -q->K * (float32_t) crealf(q->z1 + q->z2);
    p->sps->b2 = q->K * (float32_t) crealf(q->z1 * q->z2);
    p->sps->a1 = (float32_t) -crealf(q->p1 + q->p2);
    p->sps->a2 = (float32_t) crealf(q->p1 * q->p2);
}
