//#############################################################################
//! \file $FOLDER_TEST$/$DEVICE$_FixedPoint_FIR16_Alt/test_fir16d_alt.c
//!
...
#if(FIR_ORDER & 0x01)   // odd
	// for the odd order test cases, init() will make the order
	// even (this is done for calculation purposes and does not
	// reflect the true order of the filter)
	if(fir.order == FIR_ORDER - 1){
		pass++;
	}else{
		fail++;
	}
