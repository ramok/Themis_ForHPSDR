;;#############################################################################
;;! \file source/fft/CFFT_f32t.asm
;;!
;;! \brief  Complex FFT using fixed size twiddle factor tables
;;! \author C2000
;;! \date   04/24/2015
;;
;; DESCRIPTION:
;;
;;  This function computes a complex FFT using twiddle tables stored in ROM
;;  (or device memory).  The input buffer must be aligned to a multiple of the
;;  FFT size.  If it is not aligned then the output buffer will yield invalid
;;  results.  If you do not wish to align the input buffer then use the
;;  CFFT_f32ut function.  Using this function will reduce cycle performance of
;;  the algorithm.
;;
;; FUNCTIONS:
;;
;;    void CFFT_f32t (CFFT_F32_STRUCT_Handle)
;;
...
;;     * Largest complex FFT possible using the tables is 1024 points
...
;===========================================================================
; Function: void CFFT_f32t(CFFT_F32_STRUCT_Handle)
;===========================================================================
