...
int16_t main(void)
{
...
#ifdef __TMS320C28XX_TMU__  //defined when --tmu_support=tmu0 in the project 
                            // properties
    iabs_SP_CV_TMU0(z, x, N2);  // inverse complex absolute value
#else
    iabs_SP_CV(z, x, N2);        // inverse complex absolute value
#endif
    __asm(" NOP");
    ...
    memset(z, 0, SIZE*sizeof(float));
    __asm(" NOP");
    iabs_SP_CV_2(z, x, N1);  // inverse complex absolute value (TMU0)
    __asm(" NOP");
