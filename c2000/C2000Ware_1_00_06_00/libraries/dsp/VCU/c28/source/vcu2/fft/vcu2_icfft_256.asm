;;*****************************************************************************
;;! \file source/vcu2/vcu2_icfft_256.asm
;;!
;;! \brief  256-pt complex inverse FFT
;;
;;  \date   Oct 3, 2013
;;! 
;;
...
;;*****************************************************************************
    .text
;;
;; \brief Calculate the 256 pt Complex Inverse FFT
;;
...
;; --XAR1,*BR0++---o----*-----o--------*--XAR2++--->
;;
_ICFFT_run256Pt_stages1and2Combined:
...
S12_LOOP_COUNT      .set S12_NBFLY - 2           ; Stage 1/2 loop count
 ...
 || VMOV32    VR2, *BR0++                        ;[VR0H:VR0L] := [R0 - R1:R0 + R1] := [VR0L - VR1L:VR0L + VR1L]
    ...
    RPTB      _ICFFT_run256Pt_stages1and2CombinedLoop, #S12_LOOP_COUNT
...
_ICFFT_run256Pt_stages1and2CombinedLoop:
...
_ICFFT_run256Pt_stages1and2CombinedEnd:
...
;;
_ICFFT_run256Pt_stages3and4Combined:
...
;;           S1            S2                S3                         S4
    ...
    ;; no_of_outer_loop = size/(2*no_of_inner_but*2)-1 = 256/(2*4*2) = 16-1 = 15
    MOVL      XAR5, #S34_OUTER_LOOP_COUNT        ; Initialize outer loop counter
                                                 ; used in BANZ
    ;;-------------------------------------------------------------------------

_ICFFT_run256Pt_stages3and4OuterLoop:
    ...
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _ICFFT_run256Pt_stages3and4InnerLoop, #S34_INNER_LOOP_COUNT
 ...
 || VMOV32    *+XAR7[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_ICFFT_run256Pt_stages3and4InnerLoop:
 ...
 || VMOV32    *XAR7++, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    BANZ      _ICFFT_run256Pt_stages3and4OuterLoop, AR5--

_ICFFT_run256Pt_stages3and4CombinedEnd:
    ;.c28_amode                                  ; change the assembler mode back to C28x
    ;CLRC      AMODE                             ; set AMODE back to C28x addressing
                                                 ; C28_AMODE allows *XARn[#3bit] addressing
...
;;
_ICFFT_run256Pt_stages5and6Combined:
...
S56_POST_INCREMENT   .set 2*3*S56_NBFLYS         ; Post-increment for all the data pointers
    ...
    ;; For Stage s & s+1, combined
    ;; no_of_inner_but = 2^(s-1) = 2^(3-1) = 4
    ;; no_of_outer_loop = size/(2*no_of_inner_but*2)-1 = 256/(2*16*2) = 4-1 = 3
    MOVL    XAR5, #S56_OUTER_LOOP_COUNT          ; Initialize outer loop counter
                                                 ; used in BANZ
    ;;-------------------------------------------------------------------------

_ICFFT_run256Pt_stages5and6OuterLoop:
    ...
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _ICFFT_run256Pt_stages5and6InnerLoop, #S56_INNER_LOOP_COUNT
    VMOV32    VR4, *XAR3++                       ; VR4 = Sin(2):Cos(2)
    VCFFT3    VR5, VR4, VR3, VR2, VR0, #0        ; VR5 = I3:R3
 || VMOV32    VR5, *+XAR4[AR1]                   ;[VR2H:VR2L] = [I2'*Cos(2) - R2'*Sin(2): R2'*Cos(2) + I2'*Sin(2)]
                                                 ;[VR0H:VR0L] = [I0':R0'] = [I0 + VR2H : R0 + VR2L]
 ...
 || VMOV32    *+XAR7[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_ICFFT_run256Pt_stages5and6InnerLoop:
 ...
 || VMOV32    *XAR7++, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    BANZ      _ICFFT_run256Pt_stages5and6OuterLoop, AR5--

_ICFFT_run256Pt_stages5and6CombinedEnd:
    ;.c28_amode                                  ; change the assembler mode back to C28x
    ;CLRC      AMODE                             ; set AMODE back to C28x addressing
                                                 ; C28_AMODE allows *XARn[#3bit] addressing
;;=============================================================================
...
;; --XAR4[AR1]---XAR3-o----*---XAR3--o--------*--XAR7[AR0]--->
;;
_ICFFT_run256Pt_stages7and8Combined:
...
S78_OUTSEP           .set  S78_INSEP             ; Output Seperation
...
S78_INNER_LOOP_COUNT .set  S78_NBFLYS-3          ; Repeat over the number of butterflies-2(first + last bfly done outside the loop, RPTB loops n+1 times)
...
S78_POST_INCREMENT   .set 2*3*S78_NBFLYS         ; Post-increment for all the data pointers
    ...
    ;; For Stage s & s+1, combined
    ;; no_of_inner_but = 2^(s-1) = 2^(3-1) = 4
    ;; no_of_outer_loop = size/(2*no_of_inner_but*2)-1 = 256/(2*16*2) = 4-1 = 1
    ;MOVL    XAR5, #S78_OUTER_LOOP_COUNT         ; Initialize outer loop counter
                                                 ; used in BANZ
                                                 ; not required here
    ;;-------------------------------------------------------------------------

_ICFFT_run256Pt_stages7and8OuterLoop:
 ...
 || VMOV32    *+XAR6[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ...
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
	MOV       ACC, #S78_OUTSEP
	ADD       @AR6, AL                           ; XAR6 += S78_OUTSEP
	ADDB      XAR0, #(S78_OUTSEP-2)              ; AR0 := AR0 + S78_OUTSEP-2(double the offset)
	
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _ICFFT_run256Pt_stages7and8InnerLoop, #S78_INNER_LOOP_COUNT
    VMOV32    VR4, *XAR3++                       ; VR4 = Sin(2):Cos(2)
    VCFFT3    VR5, VR4, VR3, VR2, VR0, #0        ; VR5 = I3:R3
 || VMOV32    VR5, *+XAR4[AR1]                   ;[VR2H:VR2L] = [I2'*Cos(2) - R2'*Sin(2): R2'*Cos(2) + I2'*Sin(2)]
                                                 ;[VR0H:VR0L] = [I0':R0'] = [I0 + VR2H : R0 + VR2L]
 ...
 || VMOV32    *--XAR6, VR1                       ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_ICFFT_run256Pt_stages7and8InnerLoop:
 ...
 || VMOV32    *--XAR7, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    ;BANZ      _ICFFT_run256Pt_stages7and8OuterLoop, AR5--
    ;not required for the last of the combined stages

_ICFFT_run256Pt_stages7and8CombinedEnd:
    ;.c28_amode                                  ; change the assembler mode back to C28x
    ;CLRC      AMODE                             ; set AMODE back to C28x addressing
                                                 ; C28_AMODE allows *XARn[#3bit] addressing
;;=============================================================================
