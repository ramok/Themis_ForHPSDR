;;*****************************************************************************
;;! \file source/vcu2/vcu2_cfft_256.asm
;;!
...
;; globals
;;*****************************************************************************
    .global _CFFT_run256Pt
    .global _CFFT_init256Pt
...
;; --XAR1,*BR0++---o----*-----o--------*--XAR2++--->
;;
_CFFT_run256Pt_stages1and2Combined:
...
S12_LOOP_COUNT      .set S12_NBFLY - 2           ; Stage 1/2 loop count
 ...
 || VMOV32    VR2, *BR0++                        ;[VR0H:VR0L] := [R0 - R1:R0 + R1] := [VR0L - VR1L:VR0L + VR1L]
    ...
    RPTB      _CFFT_run256Pt_stages1and2CombinedLoop, #S12_LOOP_COUNT
...
_CFFT_run256Pt_stages1and2CombinedLoop:
...
_CFFT_run256Pt_stages1and2CombinedEnd:
...
;;
_CFFT_run256Pt_stages3and4Combined:
...
_CFFT_run256Pt_stages3and4OuterLoop:
    ...
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _CFFT_run256Pt_stages3and4InnerLoop, #S34_INNER_LOOP_COUNT
 ...
 || VMOV32    *+XAR7[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_CFFT_run256Pt_stages3and4InnerLoop:
 ...
 || VMOV32    *XAR7++, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    BANZ      _CFFT_run256Pt_stages3and4OuterLoop, AR5--

_CFFT_run256Pt_stages3and4CombinedEnd:
    ;.c28_amode                                  ; change the assembler mode back to C28x
    ;CLRC      AMODE                             ; set AMODE back to C28x addressing
                                                 ; C28_AMODE allows *XARn[#3bit] addressing
...
;; --XAR4[AR1]---XAR3-o----*---XAR3--o--------*--XAR7[AR0]--->
;;
_CFFT_run256Pt_stages5and6Combined:
...
_CFFT_run256Pt_stages5and6OuterLoop:
    ...
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _CFFT_run256Pt_stages5and6InnerLoop, #S56_INNER_LOOP_COUNT
    VMOV32    VR4, *XAR3++                       ; VR4 = Sin(2):Cos(2)
    VCFFT3    VR5, VR4, VR3, VR2, VR0, #1        ; VR5 = I3:R3
 || VMOV32    VR5, *+XAR4[AR1]                   ;[VR2H:VR2L] = [I2'*Cos(2) - R2'*Sin(2): R2'*Cos(2) + I2'*Sin(2)]
                                                 ;[VR0H:VR0L] = [I0':R0'] = [I0 + VR2H : R0 + VR2L]
 ...
 || VMOV32    *+XAR7[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_CFFT_run256Pt_stages5and6InnerLoop:
 ...
 || VMOV32    *XAR7++, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    BANZ      _CFFT_run256Pt_stages5and6OuterLoop, AR5--

_CFFT_run256Pt_stages5and6CombinedEnd:
    ;.c28_amode                                  ; change the assembler mode back to C28x
    ;CLRC      AMODE                             ; set AMODE back to C28x addressing
                                                 ; C28_AMODE allows *XARn[#3bit] addressing
;;=============================================================================
...
;; --XAR4[AR1]---XAR3-o----*---XAR3--o--------*--XAR7[AR0]--->
;;
_CFFT_run256Pt_stages7and8Combined:
...
_CFFT_run256Pt_stages7and8OuterLoop:
    ...
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _CFFT_run256Pt_stages7and8InnerLoop, #S78_INNER_LOOP_COUNT
    VMOV32    VR4, *XAR3++                       ; VR4 = Sin(2):Cos(2)
    VCFFT3    VR5, VR4, VR3, VR2, VR0, #1        ; VR5 = I3:R3
 || VMOV32    VR5, *+XAR4[AR1]                   ;[VR2H:VR2L] = [I2'*Cos(2) - R2'*Sin(2): R2'*Cos(2) + I2'*Sin(2)]
                                                 ;[VR0H:VR0L] = [I0':R0'] = [I0 + VR2H : R0 + VR2L]
 ...
 || VMOV32    *+XAR7[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_CFFT_run256Pt_stages7and8InnerLoop:
 ...
 || VMOV32    *XAR7++, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    ;BANZ      _CFFT_run256Pt_stages7and8OuterLoop, AR5--
    ;not required for the last of the combined stages

_CFFT_run256Pt_stages7and8CombinedEnd:
    ;.c28_amode                                  ; change the assembler mode back to C28x
    ;CLRC      AMODE                             ; set AMODE back to C28x addressing
                                                 ; C28_AMODE allows *XARn[#3bit] addressing
;;=============================================================================
...
;;*****************************************************************************
;;
;; \brief Initialize the 256-pt Complex FFT
;;
;; \param Handle to the structure, CFFT_Obj(passed in XAR4)
;; - *+XAR4[0]:  int16_t *pInBuffer -> input pointer
;; - *+XAR4[2]:  int16_t *pOutBuffer -> work(Output) buffer pointer
