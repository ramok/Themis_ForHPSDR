;;*****************************************************************************
;;! \file source/vcu2/vcu2_icfft_1024.asm
;;!
;;! \brief  1024-pt complex inverse FFT
;;
;;  \date   Oct 3, 2013
;;! 
;;
...
NSKIP               .set    2*(1024 / NSAMPLES)  ;vcu0 largest table is stage9 i.e. 512 twiddles(used in unpack)
                                                 ;max RFFT supported is 1024 points, cant use a CFFT of 1024
                                                 ;to do an RFFT of 2048, the tables aren't present in memory
                                                 ;WARNING: Do not use this skip factor in any calculations
STAGE1              .set    1
STAGE3              .set    3
STAGE5              .set    5
STAGE7              .set    7
...
;;*****************************************************************************
    .text
;;
;; \brief Calculate the 1024 pt Complex Inverse FFT
;;
...
;; --XAR1,*BR0++---o----*-----o--------*--XAR2++--->
;;
_ICFFT_run1024Pt_stages1and2Combined:
...
S12_LOOP_COUNT      .set S12_NBFLY - 2           ; Stage 1/2 loop count
 ...
 || VMOV32    VR2, *BR0++                        ;[VR0H:VR0L] := [R0 - R1:R0 + R1] := [VR0L - VR1L:VR0L + VR1L]
    ...
    RPTB      _ICFFT_run1024Pt_stages1and2CombinedLoop, #S12_LOOP_COUNT
...
_ICFFT_run1024Pt_stages1and2CombinedLoop:
...
_ICFFT_run1024Pt_stages1and2CombinedEnd:
...
;;
_ICFFT_run1024Pt_stages3and4Combined:
...
;;           S1            S2                S3                         S4
    ...
    ;; no_of_outer_loop = size/(2*no_of_inner_but*2)-1 = 1024/(2*4*2) = 64-1 = 63
    MOVL      XAR5, #S34_OUTER_LOOP_COUNT        ; Initialize outer loop counter
                                                 ; used in BANZ
    ;;-------------------------------------------------------------------------

_ICFFT_run1024Pt_stages3and4OuterLoop:
    ...
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _ICFFT_run1024Pt_stages3and4InnerLoop, #S34_INNER_LOOP_COUNT
 ...
 || VMOV32    *+XAR7[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_ICFFT_run1024Pt_stages3and4InnerLoop:
 ...
 || VMOV32    *XAR7++, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    BANZ      _ICFFT_run1024Pt_stages3and4OuterLoop, AR5--

_ICFFT_run1024Pt_stages3and4CombinedEnd:
    ;.c28_amode                                   ; change the assembler mode back to C28x
    ;CLRC      AMODE                              ; set AMODE back to C28x addressing
                                                  ; C28_AMODE allows *XARn[#3bit] addressing
...
;; --XAR4[AR1]---XAR3-o----*---XAR3--o--------*--XAR7[AR0]--->
;;
_ICFFT_run1024Pt_stages5and6Combined:
...
S56_POST_INCREMENT   .set 2*3*S56_NBFLYS         ; Post-increment for all the data pointers
    ...
    ;; For Stage s & s+1, combined
    ;; no_of_inner_but = 2^(s-1) = 2^(3-1) = 4
    ;; no_of_outer_loop = size/(2*no_of_inner_but*2)-1 = 1024/(2*16*2) = 16-1 = 15
    MOVL      XAR5, #S56_OUTER_LOOP_COUNT        ; Initialize outer loop counter
                                                 ; used in BANZ
    ;;-------------------------------------------------------------------------

_ICFFT_run1024Pt_stages5and6OuterLoop:
    ...
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _ICFFT_run1024Pt_stages5and6InnerLoop, #S56_INNER_LOOP_COUNT
    VMOV32    VR4, *XAR3++                       ; VR4 = Sin(2):Cos(2)
    VCFFT3    VR5, VR4, VR3, VR2, VR0, #0        ; VR5 = I3:R3
 || VMOV32    VR5, *+XAR4[AR1]                   ;[VR2H:VR2L] = [I2'*Cos(2) - R2'*Sin(2): R2'*Cos(2) + I2'*Sin(2)]
                                                 ;[VR0H:VR0L] = [I0':R0'] = [I0 + VR2H : R0 + VR2L]
 ...
 || VMOV32    *+XAR7[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_ICFFT_run1024Pt_stages5and6InnerLoop:
 ...
 || VMOV32    *XAR7++, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    BANZ      _ICFFT_run1024Pt_stages5and6OuterLoop, AR5--

_ICFFT_run1024Pt_stages5and6CombinedEnd:
    ;.c28_amode                                  ; change the assembler mode back to C28x
    ;CLRC      AMODE                             ; set AMODE back to C28x addressing
                                                 ; C28_AMODE allows *XARn[#3bit] addressing
;;=============================================================================
...
;; --XAR4[AR1]---XAR3-o----*---XAR3--o--------*--XAR7[AR0]--->
;;
_ICFFT_run1024Pt_stages7and8Combined:
...
S910_TFOFFSET        .set  S78_TFOFFSET+4*S78_NBFLYS
                                                 ; Twiddle factor table offset for stages 7 and 8
...
_ICFFT_run1024Pt_stages7and8OuterLoop:
    ...
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _ICFFT_run1024Pt_stages7and8InnerLoop, #S78_INNER_LOOP_COUNT
    VMOV32    VR4, *XAR3++                       ; VR4 = Sin(2):Cos(2)
    VCFFT3    VR5, VR4, VR3, VR2, VR0, #0        ; VR5 = I3:R3
 || VMOV32    VR5, *+XAR4[AR1]                   ;[VR2H:VR2L] = [I2'*Cos(2) - R2'*Sin(2): R2'*Cos(2) + I2'*Sin(2)]
                                                 ;[VR0H:VR0L] = [I0':R0'] = [I0 + VR2H : R0 + VR2L]
 ...
 || VMOV32    *+XAR7[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_ICFFT_run1024Pt_stages7and8InnerLoop:
 ...
 || VMOV32    *XAR7++, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    BANZ      _ICFFT_run1024Pt_stages7and8OuterLoop, AR5--

_ICFFT_run1024Pt_stages7and8CombinedEnd:
    ;.c28_amode                                  ; change the assembler mode back to C28x
    ;CLRC      AMODE                             ; set AMODE back to C28x addressing
                                                 ; C28_AMODE allows *XARn[#3bit] addressing
;;=============================================================================
;;
;; Stages 9 and 10 Combined
;;
;; Notes:
;; - These stages will use twiddle factors from the table, which are organized
;;   as follows. Twiddles for stages 9 and 10 are interleaved
;;   exp(2*pi*k1/N9)  , k1 = {0,1,...N9/2-1}, N9 = 2^9
;;   exp(2*pi*k2/N10) , k2 = {0,1,...N9/2-1}, N10 = 2^10
;; - Stages 9 and 10 are affected by the SHR or SHL modifiers, results from all
;;   operations are shifted right by SHR or left by SHL values
;; - XAR2, the pointer to the inputs, now points to the input buffer
;; - A lot of the constants are >#7 bit immediates and cant be used in an ADDB
;;   instruction. Additional overhead incurred from calculating offsets in the
;;   ACC first.
;;
;; Register Usage:
;;   XAR0:         butterfly lower ouput storage offset
...
;;
_ICFFT_run1024Pt_stages9and10Combined:

;; local defines
S910_INPUT_OFFSET     .set  ARG_INBUFFER
S910_OUTPUT_OFFSET    .set  ARG_OUTBUFFER

S910_NBFLYS           .set  1<<(STAGE9-1)        ; Number of butterflys per group (2^(s-1))
S910_NGROUPS          .set  NSAMPLES/(2*S910_NBFLYS)
                                                 ; Number of groups for this stage
S910_INSEP            .set  2*(S910_NBFLYS)      ; Input Seperation = (NBFLYs) * 2(size of complex inputs)
S910_OUTSEP           .set  S910_INSEP           ; Output Seperation
S910_GROUPSEP         .set  4*S910_NBFLYS        ; Seperation between the groups =  NBFLYs * 2(inputs per butterfly) * 2(size of complex inputs)
S1112_TFOFFSET        .set  S910_TFOFFSET+4*S910_NBFLYS
                                                 ; Twiddle factor table offset for stages 7 and 8

S910_INNER_LOOP_COUNT .set  S910_NBFLYS-3        ; Repeat over the number of butterflies-2(first + last bfly done outside the loop, RPTB loops n+1 times)
S910_OUTER_LOOP_COUNT .set  NSAMPLES/(4*S910_NBFLYS) - 1
                                                 ; Outer loop count = N/(2(inputs per bfly)*2(words/input)*NBFLYS) - 1
S910_POST_INCREMENT   .set 2*3*S910_NBFLYS       ; Post-increment for all the data pointers
    ...
    ;; Note that previous stage's output array is input for this stage
    MOVL      XAR4, *-SP[STK_ARG_PTR]            ; Restore the pointer argument to XAR4
    MOVL      XAR2, *+XAR4[S910_INPUT_OFFSET]    ; XAR2 -> I0:R0 pointer
    ;;-------------------------------------------------------------------------
    ...
    ;;  For Stage 7 & 8, input_separation = 2 * 2^(7-1) = 128
    ;; And so on ....
    MOVL      XAR1, #S910_INSEP

    ;; ar0 is added with an XARn pointer which is post incremented (+2)
    ;; and hence ar0 = xar1-2
    MOVL      XAR0, #S910_OUTSEP
    ;;-------------------------------------------------------------------------
    ...
    ;; s & s+1 stage butterfly. Note that the input buffer of the previous
    ;; stage is used as output buffer for this stage due to ping-pong scheme
    MOVL      XAR6, *+XAR4[S910_OUTPUT_OFFSET]   ; XAR6 -> first output
                                                 ; I0'':R0'' pointer
    ;;-------------------------------------------------------------------------
    ...
    ;;And so on ...
    MOVL      ACC, XAR6
    ADD		  ACC, #S910_GROUPSEP
    MOVL	  XAR7, ACC						     ; I2'':R2'' pointer
    ;;-------------------------------------------------------------------------
    ...
    ;; no_of_inner_but = 2^(s-1) = 2^(3-1) = 4
    ;; no_of_outer_loop = size/(2*no_of_inner_but*2)-1 = 1024/(2*16*2) = 16-1 = 15
    ;MOVL    XAR5, #S910_OUTER_LOOP_COUNT        ; Initialize outer loop counter
                                                 ; used in BANZ
                                                 ; not required here
    ;;-------------------------------------------------------------------------

_ICFFT_run1024Pt_stages9and10OuterLoop:
 ...
 || VMOV32    *+XAR6[AR0], VR1                   ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ...
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
	MOV       ACC, #S910_OUTSEP
	ADD       @AR6, AL                           ; XAR6 += S910_OUTSEP
	ADD       @AR0, AL                           ; AR0 := AR0 + S910_OUTSEP-2(double the offset)
	SUBB	  XAR0, #2
	
    .align    2                                  ; align at 32-bit boundary to remove penalty
    RPTB      _ICFFT_run1024Pt_stages9and10InnerLoop, #S910_INNER_LOOP_COUNT
    VMOV32    VR4, *XAR3++                       ; VR4 = Sin(2):Cos(2)
    VCFFT3    VR5, VR4, VR3, VR2, VR0, #0        ; VR5 = I3:R3
 || VMOV32    VR5, *+XAR4[AR1]                   ;[VR2H:VR2L] = [I2'*Cos(2) - R2'*Sin(2): R2'*Cos(2) + I2'*Sin(2)]
                                                 ;[VR0H:VR0L] = [I0':R0'] = [I0 + VR2H : R0 + VR2L]
 ...
 || VMOV32    *--XAR6, VR1                       ;[VR2H:VR2L] = [I1*Cos(1) - R1*Sin(1):R1*Cos(1) + I1*Sin(1)]
                                                 ;[VR0H:VR0L] = [I2':R2'] = [I2 + VR2H: R2 + VR2L]
                                                 ;[VR1H:VR1L] = [I3':R3'] = [I2 - VR2H: R2 - VR2L]
_ICFFT_run1024Pt_stages9and10InnerLoop:
 ...
 || VMOV32    *--XAR7, VR1                       ;[VR0H:VR0L] = [I1'':R1'’] = [I1' - VR2H: R1' + VR2L]
    ...
    ;BANZ      _ICFFT_run1024Pt_stages9and10OuterLoop, AR5--
    ;not required for the last of the combined stages

_ICFFT_run1024Pt_stages9and10CombinedEnd:
    ;.c28_amode                                  ; change the assembler mode back to C28x
    ;CLRC      AMODE                             ; set AMODE back to C28x addressing
                                                 ; C28_AMODE allows *XARn[#3bit] addressing
;;=============================================================================
