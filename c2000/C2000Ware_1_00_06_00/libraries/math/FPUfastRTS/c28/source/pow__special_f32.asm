...
;;
;;#############################################################################
;; $TI Release: C2000Ware 1.00.05.00 $
...
;; external references
    ...
    .asg    _powf, _powf_special
    .asg    powf_special, _powf_special
    .endif
    .ref    _FPU32logTable
    .ref    _FPU32LOG2
    .ref    _FPU32expTable
    .ref    _FPU32expTableCoeff
    .export _powf_special
...
;;-----------------------------------------------------------------------------
    ...
    .if __TI_EABI__
    ;; The assembler does the addition of FPUlogTable and 6 prior to
	...
	ADD			ACC, LF_X_MANT << #1 ;          += 2*(int)(Xm*32.0)
									 ;      ACC  -> FPUlogTable[idx].A2+2
    MOVL	    XAR0, ACC			 ;      XAR0 -> FPUlogTable[idx].A2+2
...
;;-----------------------------------------------------------------------------
 ...
 || MOV32       R0H, *XAR1++         ;  | | R0H = 1/3
    ...
    MOV32        ACC, R4H            ; 1|   ACC = sizeof(float)*x_i
    ...
    ADDF32      R2H, #1.0, R2H       ; *|1| 
    ADD         ACC, #_FPU32expTable   ; 1|   ACC->FPUexpTable[x_i]
    ; R2H = x_f/2*(1+x_f/3*(1+x_f/4*(1+x_f/5*(1+x_f/6*(1+x_f/7)))))
    MPYF32      R2H, R3H, R2H        ; *|   
    MOVL        XAR0, ACC            ; 1|   XAR0->FPUexpTable[x_i]
    ...
    MPYF32      R2H, R1H, R2H        ; *|
    MOV32       R1H, *XAR0           ; 1|   R1H = FPUexpTable[x_i]
