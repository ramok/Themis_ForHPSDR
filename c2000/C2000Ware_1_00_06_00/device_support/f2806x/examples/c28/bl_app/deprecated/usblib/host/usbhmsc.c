                 ...
                 unsigned char *pucMaxLUN)
{
    ...
    //
    writeShort(&(SetupPacket.wLength), 1);
...
USBHMSCDriveReady(unsigned long ulInstance)
{
    ...
    else
    {
        ...
        pMSCDevice->ulBlockSize =
            (pBuffer[7] | ((unsigned long)pBuffer[6] << 8) | ((unsigned long)pBuffer[5] << 16) |
             ((unsigned long)pBuffer[4] << 24));

        //
        // Read the block size out.
        //
        pMSCDevice->ulNumBlocks =
            (pBuffer[3] | ((unsigned long)pBuffer[2] << 8) | ((unsigned long)pBuffer[1] << 16) |
             ((unsigned long)pBuffer[0] << 24));
    }
