...
{
	sizeof(g_pHIDDescriptor) & 0xFF,
	(sizeof(g_pHIDDescriptor) & 0xFF00) >> 8,
    g_pHIDDescriptor
};

const tConfigSection g_sHIDInterfaceSection =
{
	sizeof(g_pHIDInterface) & 0xFF,
	(sizeof(g_pHIDInterface) & 0xFF00) >> 8,
    g_pHIDInterface
};

const tConfigSection g_sHIDInEndpointSection =
{
	sizeof(g_pHIDInEndpoint) & 0xFF,
	(sizeof(g_pHIDInEndpoint) & 0xFF00) >> 8,
    g_pHIDInEndpoint
};

const tConfigSection g_sHIDOutEndpointSection =
{
	sizeof(g_pHIDOutEndpoint) & 0xFF,
	(sizeof(g_pHIDOutEndpoint) & 0xFF00) >> 8,
    g_pHIDOutEndpoint
};
...
//    HWREGBITH(pusDeferredOp, usBit) = bSet ? 1 : 0;
    HWREGH(pusDeferredOp) = (HWREGH(pusDeferredOp) & ~(1 << usBit)) | ((bSet ? 1 : 0)<<usBit);
}
            ...
            ulLoop++)
    {
        ...
        if(psDesc->bDescriptorType == ucType)
        {
            ...
            if(ulCount == ulIndex)
            {
                ...
                // caller.
                //
                *pulLen = (unsigned long)readShort(&(psDesc->wDescriptorLength));
                return(ulLoop);
            }
            else
            {
...
ScheduleReportTransmission(tHIDInstance *psInst)
{
    ...
    //
    lRetcode = USBEndpointDataPut(psInst->ulUSBBase, psInst->ucINEndpoint,
                                  pucData, ulNumBytes);
    ...
    if(lRetcode != -1)
    {
        ...
        //
        lRetcode = USBEndpointDataSend(psInst->ulUSBBase,
                                           psInst->ucINEndpoint,
                                       USB_TRANS_IN);
    }
...
HandleGetDescriptor(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    case USB_HID_DTYPE_PHYSICAL:
    {
        ...
        //
        ulSize = 0;
        ulDesc = FindHIDDescriptor(psDevice, readShort(&(pUSBRequest->wValue)) >> 8,
        ...
        //
        if(ulSize > readShort(&(pUSBRequest->wLength)))
        {
            ulSize = readShort(&(pUSBRequest->wLength));
        }
...
HandleRequest(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    case USBREQ_SET_REPORT:
    {
        ...
        else
        {
            ...
            USBDCDRequestDataEP0(0, psInst->pucOutReportData,
                                 (unsigned long)readShort(&(pUSBRequest->wLength)));
    ...
    case USBREQ_GET_REPORT:
    {
        ...
        ulSize = psDevice->pfnRxCallback(psDevice->pvRxCBData,
                                         ...
                                         readShort(&(pUSBRequest->wValue)), &pucReport);
    ...
    case USBREQ_SET_IDLE:
    {
        ...
        //
        SetIdleTimeout(psDevice, readShort(&(pUSBRequest->wValue)) & 0xFF,
                       (readShort(&(pUSBRequest->wValue)) >> 8) & 0xFF);
    ...
    case USBREQ_GET_IDLE:
    {
        ...
        //
        ulTimeout = GetIdleTimeout(psDevice, readShort(&(pUSBRequest->wValue)));
...
USBDHIDCompositeInit(unsigned long ulIndex, const tUSBDHIDDevice *psDevice)
{
    ...
    writeShort(&(psDevDesc->idVendor), psDevice->usVID);
    writeShort(&(psDevDesc->idProduct), psDevice->usPID);
    ...
    // descriptor.
    //
    writeShort(&(g_sHIDDescriptorSection.usSize), psDevice->psHIDDescriptor->bLength);
