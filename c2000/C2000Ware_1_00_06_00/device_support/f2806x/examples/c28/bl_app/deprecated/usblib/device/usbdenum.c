...
//! C2000 microcontrollers.  Consult the data sheet for the microcontroller
...
USBDGetStatus(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    case USB_RTYPE_ENDPOINT:
    {
        ...
        //
        usIndex = readShort(&(pUSBRequest->wIndex)) & USB_REQ_EP_NUM_M;
        ...
        else
        {
            ...
            //
            ulDir = ((readShort(&(pUSBRequest->wIndex)) & USB_REQ_EP_DIR_M) ==
...
USBDClearFeature(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    case USB_RTYPE_DEVICE:
    {
        ...
        //
        if(USB_FEATURE_REMOTE_WAKE & readShort(&(pUSBRequest->wValue)))
        {
            //
    ...
    case USB_RTYPE_ENDPOINT:
    {
        ...
        else
        {
            ...
            //
            if(USB_FEATURE_EP_HALT == readShort(&(pUSBRequest->wValue)))
            {
                //
                // Are we dealing with an IN or OUT endpoint?
                //
...
USBDSetAddress(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    // phase is complete.
    //
    psUSBControl->ulDevAddress = readShort(&(pUSBRequest->wValue)) | DEV_ADDR_PENDING;
...
USBDGetDescriptor(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    //
    switch(readShort(&(pUSBRequest->wValue)) >> 8)
    {
        //
    ...
    case USB_DTYPE_CONFIGURATION:
    {
        ...
        //
        ucIndex = (unsigned char)(readShort(&(pUSBRequest->wValue)) & 0xFF);
    ...
    case USB_DTYPE_STRING:
    {
        ...
        //
        lIndex = USBDStringIndexFromRequest(readShort(&(pUSBRequest->wIndex)),
                                            readShort(&(pUSBRequest->wValue)) & 0xFF);
    ...
    if(psUSBControl->pEP0Data)
    {
        ...
        //
        if(psUSBControl->ulEP0DataRemain > readShort(&(pUSBRequest->wLength)))
        {
            psUSBControl->ulEP0DataRemain = readShort(&(pUSBRequest->wLength));
        }
...
USBDStringIndexFromRequest(unsigned short usLang, unsigned short usIndex)
{
    ...
    for(ulLoop = 0; ulLoop < ulNumLangs; ulLoop++)
    {
        ...
        //
        if(readShort(&(pLang->wLANGID)+ 2*ulLoop) == usLang)
        {
            //
...
USBDSetConfiguration(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    //
    if(readShort(&(pUSBRequest->wValue)) > psUSBControl->psInfo->pDeviceDescriptor[17])
    {
        //
    ...
    else
    {
        ...
        //
        psUSBControl->ulConfiguration = readShort(&(pUSBRequest->wValue));
        ...
        if(psUSBControl->ulConfiguration)
        {
            ...
            //
            psHdr = psDevice->ppConfigDescriptors[readShort(&(pUSBRequest->wValue)) - 1];
            psDesc = (const tConfigDescriptor *)(psHdr->psSections[0]->pucData);
            ...
            USBDeviceConfig(psUSBControl,
                         psDevice->ppConfigDescriptors[readShort(&(pUSBRequest->wValue)) - 1]);
        }
...
USBDGetInterface(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    else
    {
        ...
        //
        if(readShort(&(pUSBRequest->wIndex)) < USB_MAX_INTERFACES_PER_DEVICE)
        {
            //
            // Read the current alternate setting for the required interface.
            //
            ucValue = psUSBControl->pucAltSetting[readShort(&(pUSBRequest->wIndex))];
        }
        else
        {
            //
...
USBDSetInterface(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    for(ulLoop = 0; ulLoop < ulNumInterfaces; ulLoop++)
    {
        ...
        //
        if(psInterface &&
                (psInterface->bInterfaceNumber == readShort(&(pUSBRequest->wIndex))) &&
                (psInterface->bAlternateSetting == readShort(&(pUSBRequest->wValue))))
        {
            ...
            if(bRetcode && psDevice->sCallbacks.pfnInterfaceChange)
            {
                psDevice->sCallbacks.pfnInterfaceChange(
                    ...
                    readShort(&(pUSBRequest->wIndex)),
                    readShort(&(pUSBRequest->wValue)));
            }
...
USBDEP0StateTxConfig(unsigned long ulIndex)
{
            ...
            (g_psUSBDevice[0].ucConfigSection == 0))
    {
        ...
        //
        writeShort(&(sConfDesc.wTotalLength), (unsigned short)USBDCDConfigDescGetSize(
                       psConfig));
        ...
        //
        if(readShort(&(psConfig->psSections[0]->usSize)) == ulToSend)
        {
            //
    ...
    while(ulToSend)
    {
        ...
        //
        ulSecBytes = (unsigned long)(readShort(&(psSection->usSize)) -
        ...
        //
        if(g_psUSBDevice[0].ucSectionOffset == readShort(&(psSection->usSize)))
        {
            //
