...
//                             UDMA_ARB_64));
                ...
                if(FIFOAlloc(&g_sUSBHCD.USBOUTPipes[lIdx], ulSize) != 0)
                {
                    ...
                    // Configure the FIFO.
                    //
                    USBFIFOConfigSet(USB0_BASE, INDEX_TO_USB_EP(lIdx + 1),
                ...
                //
                USBHostAddrSet(USB0_BASE, INDEX_TO_USB_EP(lIdx + 1),
                ...
                unsigned long ulSize)
{
    ...
    while(ulRemainingBytes != 0)
    {
                ...
                (g_bUseDMAWA && (ulByteToSend != 64)))
        {
            ...
            //
            USBEndpointDataPut(USB0_BASE, ulEndpoint, pucData,
                                   ulByteToSend);
...
//            uDMAChannelEnable(UDMA_CHANNEL_USBEP1TX + (ulPipeIdx * 2));
        ...
        while(g_sUSBHCD.USBOUTPipes[ulPipeIdx].eState == PIPE_WRITING)
        {
            ...
            if(ulEPStatus & USB_HOST_OUT_STALL)
            {
                ...
                if(ulPipe & EP_PIPE_USE_UDMA)
                {
//                    uDMAChannelDisable(UDMA_CHANNEL_USBEP1TX +
//                                           (ulPipeIdx * 2));
                }
            }
        ...
        while(g_sUSBHCD.USBINPipes[ulPipeIdx].eState == PIPE_READING)
        {
            ...
            if(ulEPStatus & USB_HOST_IN_STALL)
            {
                ...
                if(ulPipe & EP_PIPE_USE_UDMA)
                {
                    ...
                    // Disable the DMA channel.
                    //
//                    uDMAChannelDisable(UDMA_CHANNEL_USBEP1RX +
//                                           (ulPipeIdx * 2));
                }
            }
...
USBHCDGetConfigDescriptor(unsigned long ulIndex, tUSBHostDevice *pDevice)
{
    ...
    writeShort(&(SetupPacket.wLength), USB_DTYPE_CONFIGURATION << 8);

    //
    // Index is always 0 for device configurations requests.
    //
    writeShort(&(SetupPacket.wIndex), 0);
    ...
    if(!pDevice->bConfigRead)
    {
        ...
        //
        writeShort(&(SetupPacket.wLength), sizeof(tConfigDescriptor));
    ...
    if(ulBytes == sizeof(tConfigDescriptor))
    {
        ...
        //
        if(readShort(&(SetupPacket.wLength)) > g_sUSBHCD.ulPoolSize)
        {
            writeShort(&(SetupPacket.wLength), g_sUSBHCD.ulPoolSize);
        }
...
USBHCDGetDeviceDescriptor(unsigned long ulIndex, tUSBHostDevice *pDevice)
{
    ...
    //
    SetupPacket.bRequest = USBREQ_GET_DESCRIPTOR;
    writeShort(&(SetupPacket.wValue), USB_DTYPE_DEVICE << 8);
    ...
    //
    writeShort(&(SetupPacket.wLength), sizeof(tDeviceDescriptor));
    ...
    {
        writeShort(&(SetupPacket.wLength), (unsigned short)sizeof(tDeviceDescriptor));
...
USBHCDSetAddress(unsigned long ulDevIndex, unsigned long ulDevAddress)
{
    ...
    writeShort(&(SetupPacket.wValue), ulDevAddress);
    ...
    // Only request the space available.
    //
    writeShort(&(SetupPacket.wLength), 0);
                   ...
                   unsigned long ulFeature)
{
    ...
    writeShort(&(SetupPacket.wValue), ulFeature);
    ...
    {
        writeShort(&(SetupPacket.wIndex), g_sUSBHCD.USBINPipes[ulIndex].ucEPNumber | 0x80);
    }
    else
    {
        writeShort(&(SetupPacket.wIndex), g_sUSBHCD.USBOUTPipes[ulIndex].ucEPNumber);
    }
                ...
                unsigned long ulConfiguration)
{
    ...
    writeShort(&(SetupPacket.wValue), ulConfiguration);
                   ...
                   unsigned long ulInterface, unsigned ulAltSetting)
{
    ...
    //
    writeShort(&(SetupPacket.wIndex), ulInterface);

    //
    // wValue is the alternate setting.
    //
    writeShort(&(SetupPacket.wValue), ulAltSetting);
...
USBHCDEnumHandler(void)
{
    ...
    case EP0_STATE_RX:
    {
        ...
        if(ulDataSize != 0)
        {
            ...
            //
                USBEndpointDataGet(USB0_BASE, USB_EP_0,
        ...
        //
            USBHostEndpointDataAck(USB0_BASE, USB_EP_0);
