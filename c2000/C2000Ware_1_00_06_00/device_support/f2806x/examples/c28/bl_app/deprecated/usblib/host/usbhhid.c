		                ...
		                USB_EP_ATTR_INT)
		        {
		            ...
		            if(pEndpointDescriptor->bEndpointAddress & USB_EP_DESC_IN)
		            {
                        ...
                        USBHCDPipeConfig(g_pHIDDevice[lDev].ulIntInPipe,
                                         readShort(&(pEndpointDescriptor->wMaxPacketSize)),
               ...
               unsigned char ucReportID)
{
    ...
    writeShort(&(SetupPacket.wValue), (ucDuration << 8) | ucReportID);
                           ...
                           unsigned long ulSize)
{
    ...
    writeShort(&(SetupPacket.wValue), USB_HID_DTYPE_REPORT << 8);
    ...
    //
    writeShort(&(SetupPacket.wLength), ulSize);
...
USBHHIDSetProtocol(unsigned long ulInstance, unsigned long ulBootProtocol)
{
    ...
    if(ulBootProtocol)
    {
        ...
        // Boot Protocol.
        //
        writeShort(&(SetupPacket.wValue), 0);
    }
    else
    {
        //
        // Report Protocol.
        //
        writeShort(&(SetupPacket.wValue), 1);
    }
                 ...
                 unsigned char *pucData, unsigned long ulSize)
{
    ...
    writeShort(&(SetupPacket.wValue), USB_HID_REPORT_OUTPUT << 8);

    //
    // Index is always 0 for device requests.
    //
    writeShort(&(SetupPacket.wIndex), (unsigned short)ulInterface);
