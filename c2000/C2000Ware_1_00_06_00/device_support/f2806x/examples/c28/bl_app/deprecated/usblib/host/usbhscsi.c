                    ...
                    unsigned long *pulSize)
{
    ...
    //
    writeLong(&(CmdStatus.dCSWSignature), 0);
    writeLong(&(CmdStatus.dCSWTag), 0);
    CmdStatus.bCSWStatus = SCSI_CMD_STATUS_FAIL;

    //
    // Set the CBW signature and tag.
    //
    writeLong(&(pSCSICmd->dCBWSignature), CBW_SIGNATURE);
    writeLong(&(pSCSICmd->dCBWTag), CBW_TAG_VALUE);

    //
    // Set the size of the data to be returned by the device.
    //
    writeLong(&(pSCSICmd->dCBWDataTransferLength), *pulSize);
    ...
    //
    if(readLong(&(pSCSICmd->dCBWDataTransferLength)) != 0)
    {
        //
    ...
    // indicate a failure.
    //
    if((ulBytes == 0) || (readLong(&(CmdStatus.dCSWSignature)) != CSW_SIGNATURE) ||
            (readLong(&(CmdStatus.dCSWTag)) != CBW_TAG_VALUE))
    {
        return(SCSI_CMD_STATUS_FAIL);
    }
