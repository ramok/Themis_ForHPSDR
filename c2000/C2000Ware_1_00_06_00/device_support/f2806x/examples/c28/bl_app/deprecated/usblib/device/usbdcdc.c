...
{
	sizeof(g_pCDCSerDescriptor) & 0xFF,
	(sizeof(g_pCDCSerDescriptor) & 0xFF00) >> 8,
    g_pCDCSerDescriptor
};
...
{
	sizeof(g_pIADSerDescriptor) & 0xFF,
	(sizeof(g_pIADSerDescriptor) & 0xFF00) >> 8,
    g_pIADSerDescriptor
};
...
{
	sizeof(g_pCDCSerCommInterface) & 0xFF,
	(sizeof(g_pCDCSerCommInterface) & 0xFF00) >> 8,
...
{
	sizeof(g_pCDCSerDataInterface) & 0xFF,
	(sizeof(g_pCDCSerDataInterface) & 0xFF00) >> 8,
...
SendSerialState(const tUSBDCDCDevice *psDevice)
{
    ...
    writeShort(&(sRequest.wValue), 0);
    writeShort(&(sRequest.wIndex), 0);
    writeShort(&(sRequest.wLength), USB_CDC_NOTIFY_SERIAL_STATE_SIZE);

    //
    // Write the request structure to the USB FIFO.
    //
    lRetcode = USBEndpointDataPut(psInst->ulUSBBase,
                                      psInst->ucControlEndpoint,
                                  (unsigned char *)&sRequest,
                                  sizeof(tUSBRequest));
    lRetcode = USBEndpointDataPut(psInst->ulUSBBase, psInst->ucControlEndpoint,
...
//
//*****************************************************************************
volatile unsigned long endCount = 0;
volatile unsigned long outCount = 0;
volatile unsigned long inCount = 0;
static void
...
HandleRequests(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    //
    if(readShort(&(pUSBRequest->wIndex)) != psInst->ucInterfaceControl)
    {
        return;
    }
    ...
    case USB_CDC_SET_CONTROL_LINE_STATE:
    {
        ...
        //
        psInst->usControlLineState = readShort(&(pUSBRequest->wValue));
    ...
    case USB_CDC_SEND_BREAK:
    {
        ...
        //
        psInst->usBreakDuration = readShort(&(pUSBRequest->wValue));
        ...
        // processed.
        //
        bRetcode = CheckAndSendBreak(psDevice, readShort(&(pUSBRequest->wValue)));
...
USBDCDCCompositeInit(unsigned long ulIndex, const tUSBDCDCDevice *psCDCDevice)
{
    ...
    writeShort(&(psDevDesc->idVendor), psCDCDevice->usVID);
    writeShort(&(psDevDesc->idProduct), psCDCDevice->usPID);
