...
USBIntStatus(unsigned long ulBase)
{
    ...
    ulStatus |= ((unsigned long)HWREGB(ulBase + USB_O_IS) << USB_INT_STATUS_SHIFT);
...
//*****************************************************************************
volatile unsigned long intStatus[2048];
volatile unsigned long intIndex = 0;
...
USBIntStatusEndpoint(unsigned long ulBase)
{
    ...
    ulStatus |= ((unsigned long) HWREGH(ulBase + USB_O_RXIS) << USB_INTEP_RX_SHIFT);

    if((ulStatus != 1) && (ulStatus != 0) && (intIndex < 2048)){
    	intStatus[intIndex++] = ulStatus;
    }
...
USBEndpointStatus(unsigned long ulBase, unsigned long ulEndpoint)
{
    ...
    //
    ulStatus |= ((unsigned long)(HWREGH(ulBase + EP_OFFSET(ulEndpoint) + USB_O_RXCSRL1)) <<
...
USBNumEndpointsGet(unsigned long ulBase)
{
    ...
    //
    // F2806x devices support 15 endpoint pairs.
    //
    return(3);
