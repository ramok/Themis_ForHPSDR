...
InterfaceToIndex(tUSBDCompositeDevice *psDevice, unsigned long ulInterface)
{
    ...
    for(ulLoop = 0; ulLoop < psDevice->ulNumDevices; ulLoop++)
    {
        ...
        // return its index.
        //
        if(ulInterface < __byte((int *)pucLookupEntry, LOOKUP_INTERFACE_BYTE))
        {
            return(ulLoop);
        }
...
GetDescriptor(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    {
        ulIdx = InterfaceToIndex(psDevice, (readShort(&(pUSBRequest->wIndex)) & 0xFF));
        break;
    }

    case USB_RTYPE_ENDPOINT:
    {
        ulIdx = EndpointToIndex(psDevice, (readShort(&(pUSBRequest->wIndex)) & 0x0F),
                                (readShort(&(pUSBRequest->wIndex)) & 0x80) ? true : false);
        break;
    }
...
BuildCompositeDescriptor(tUSBDCompositeDevice *psCompDevice)
{
    ...
    writeShort(&(psCompDevice->psPrivateData->ppsCompSections[0]->usSize),
    ...
    // list.
    //
    writeShort(&(psCompDevice->psPrivateData->ppsCompSections[1]->usSize), 0);
    ...
    while(ulDev < psCompDevice->ulNumDevices)
    {
        ...
        for(ulIdx = 0; ulIdx < pConfigHeader->ucNumSections; ulIdx++)
        {
            ...
            else
            {
                ...
                // it entirely.
                //
                if(readShort(&(pConfigHeader->psSections[ulIdx]->usSize)) <= usBytes)
                {
                    continue;
                }
            }
            ...
            //
            for(ulCPIdx = 0;
                    ulCPIdx < readShort(&(pConfigHeader->psSections[ulIdx]->usSize));
                    ulCPIdx++)
            {
            ...
            //
            while(usBytes < readShort(&(pConfigHeader->psSections[ulIdx]->usSize)))
            {
                //
            ...
            ulOffset += readShort(&(pConfigHeader->psSections[ulIdx]->usSize));
        ...
        psCompDevice->pulDeviceWorkspace[ulDev] =
            ...
            ((unsigned long)ucOUTEndpoint << (LOOKUP_OUT_END_BYTE * 8)) |
            ((unsigned long)ucINEndpoint << (LOOKUP_IN_END_BYTE * 8));
    ...
    writeShort(&(psCompDevice->psPrivateData->ppsCompSections[1]->usSize), ulOffset);
    psCompDevice->psPrivateData->sConfigDescriptor.bNumInterfaces =
        ucInterface;
    writeShort(&(psCompDevice->psPrivateData->sConfigDescriptor.wTotalLength), usTotalLength);
                  ...
                  unsigned long ulSize, unsigned char *pucData)
{
    ...
    //
    writeShort(&(psInst->sDeviceDescriptor.idVendor), psDevice->usVID);
    writeShort(&(psInst->sDeviceDescriptor.idProduct), psDevice->usPID);
