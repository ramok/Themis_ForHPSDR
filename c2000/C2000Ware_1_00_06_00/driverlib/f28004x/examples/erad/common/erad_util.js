...
{
    //
    // Based on the expected input, there will be six characters between the
    // beginning of the string and the start of the address
    //  
    var numSpacesToAddress = 6; 
    
    //
    // Based on the expected input, the first character is the page number
    //
    var pageNumber = Integer.parseInt(funcStr.charAt(0));
    
    //
    // Remove the portion of the string that contains the page number, making
    // the beginning of the string start at the address value
    //
    var strNoPageValue =
        funcStr.substring(numSpacesToAddress, funcStr.length());

    //
    // Remove function name from the string. Function name will always start
    // with "_". Remove space after address as well
    //
    var strNoFuncName =
        strNoPageValue.substring(0, strNoPageValue.indexOf("_") - 2);
    
    //
    // Address is a hex value
    //
    return Integer.parseInt(strNoFuncName, 16);
}
...
function getFuncPageNo(funcStr)
{
    ...
    //
    if(funcStr.length() > 0 && !funcStr.contains("abs"))
    {
